...
if [ ! -f ../conf/patterns.txt ]; then
	...
	gawk -F '\t' -v gloc=$Genomes_folder -v fuzloc=$fuzzPro_folder -v fuzz=$fuzzPro_env -v user=$userMail -v useClust=$use_cluster -v sgeQueue=$sgeQueue "BEGIN{
		i=1;
		j=1;
		...
		if(i==1){
			...
			for (j in genomes){
				...
				if(toupper(useClust) == \"YES\"){
					...
					cmd4= \". ~/.bashrc;\";
					cmd4= cmd4 \"conda activate \" fuzz \";\";
					cmd4= cmd4 \" time fuzzpro -sequence \" gloc \"/\" fasta[j] \" -pattern \" \$2 \" -outfile \" fuzloc \"/\" genomes[j] \"_vs_\" \$1 \".fuzz.txt;\" ;
					cmd4= cmd4 \"conda deactivate;\";
					print cmd4;
					#system(cmd4);
				}else{
					#cmd4= \". /path/to/anaconda3/etc/profile.d/conda.sh;\";
					cmd4= \". ~/.bashrc;\";
					cmd4= cmd4 \"conda activate \" fuzz \";\";
					cmd4= cmd4 \" time fuzzpro -sequence \" gloc \"/\" fasta[j] \" -pattern \" \$2 \" -outfile \" fuzloc \"/\" genomes[j] \"_vs_\" \$1 \".fuzz.txt;\" ;
					cmd4= cmd4 \"conda deactivate;\";
